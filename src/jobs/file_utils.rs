...
pub fn get_jobs(dir: &Path) -> io::Result<HashMap<String, Vec<PathBuf>>> {
...
fn get_paths_in_dir(path: &Path) -> io::Result<Vec<PathBuf>> {
    return Ok(std::fs::read_dir(path)?.filter_map(|entry| {
        match entry {
